...
- `sync-index.yml` is the **single owner** of regenerating the packwiz-generated `index.toml`/`pack.toml` on a PR (it folded together the old `resolve-conflicts.yml` + `refresh-index.yml`, which overlapped and could race — #127; one workflow + per-PR concurrency can't race itself). On push to `main`, PR open/reopen/**synchronize**, or dispatch it: merges `main` if the branch is behind and rebuilds the index from the combined content (auto-resolving **only** regenerable conflicts; **bails to a human** on real content conflicts, or when `pack.toml` differs beyond the `[index]` hash); and, for a branch that's merely **stale** (edited a tracked file, forgot `packwiz refresh`), auto-commits **pure hash-drift only** with `[skip ci]`, bailing on file-list changes so the `.packwizignore` guard stands. **Manual fix when it bails:** merge `main`, then either `packwiz refresh` + commit, or — for a docs-only PR — `git checkout origin/main -- index.toml pack.toml`. Steps in `docs/CI-CHECKS.md`. The push retry is the shared `push-with-retry` composite action. The old browser **dispatch mod-management workflows (add-mod / remove-mod / update-mods / compute-hash / refresh) were retired** — the editor does all of it now (#127).

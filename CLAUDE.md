...
- `refresh-index.yml` owns PR **`synchronize`** — auto-refreshes the index when you edit a tracked pack file and push without `packwiz refresh` (pure hash-drift only; it bails on file-list changes so the `.packwizignore` guard stands). `resolve-conflicts.yml` owns open/reopen + push-to-main, so the two never race a push (the split is #127). Both share the `push-with-retry` composite action; the old browser **dispatch mod-management workflows (add-mod / remove-mod / update-mods / compute-hash / refresh) were retired** — the editor does all of it now (#127).

...
1. Land the new pass so `phase2/CANDIDATES.md` is regenerated (the master accumulator).
2. Run:
   ```
   python3 tools/weave-ledger/gen-weave-review.py
   ```
...
| `phase2/CANDIDATES.md` | the corpus (input; regenerated by the pass pipeline, not by hand) |

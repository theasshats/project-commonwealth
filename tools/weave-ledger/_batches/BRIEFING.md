...
cooperative Create pack). The goal of the whole project: the pack should read as **"one or two cohesive
webs, not many clusters"** — every mod tied into **≥2 of five systems**. Your job is the *understanding*
...
## The five systems (pillars) — every mod should anchor to ≥1, ideally 2
1. **Create** — the tech spine; things made through Create machines/parts.
2. **magic** — Ars Nouveau / Iron's Spellbooks / Occultism / Forbidden Arcanus.
3. **economy** — Numismatics coins + trade (sellable goods, bounties).
4. **aeronautics** — Create Aeronautics ships/planes, trains, drones, logistics/transport.
5. **survival** — Serene Seasons × Cold Sweat × Spice of Life food/seasons; the world pushing back.
Sanctioned **support roles** (count as a valid anchor, need no weave): library/API, performance, QoL,
client-only UI, decoration palette, curated friend-group flavor.
...
spirit_fire/ritual) · M-12 Processing-chain pull (farmersdelight/extradelight/create milling). Full list:
`docs/WEAVE-LEDGER.md`. Methods you can route through: `tools/weave-ledger/methods-palette.md`.

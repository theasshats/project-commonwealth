...
MILE_LABEL = {
    ...
    'v0.9.0':  'v0.9.0 — Economy & logistics (incl. aeronautics)',
    'v0.11.0': 'v0.11.0 — Magic',
    'v0.13.0': 'v0.13.0 — Survival',
}
MILE_ORDER = ['v0.7.0', 'v0.9.0', 'v0.11.0', 'v0.13.0']
CANON = ['create', 'magic', 'economy', 'survival', 'aeronautics']

def pillars_of(anchors_txt):
    """Distinct canonical pillars in the 'anchors after:' text. Truncate before the
    first '(' or '|' (so parenthetical prose isn't counted), split on +/,/ separators,
    and substring-match each token to a canonical pillar (robust to 'Create',
    'survival only', 'economy×2'). Identical logic to consolidate.py so the two docs
    agree on every mod's anchor count."""
    head = re.split(r'[(|]', anchors_txt)[0]
    pil = []
    ...
    for f in sorted(glob.glob(os.path.join(HERE, 'batch-*.md'))):
        ...
        for ln in open(f):
            ...
            if cur and DEC.match(ln):
                ...
                ml = MILE.search(rest); mile = ml.group(0) if ml else ''
...
def build(mods):
    ...
    o = []
    o.append('# Derpack X — weave triage: review & ratification\n')
    ...
    o.append('- **§1 KEEP ratification** is the main event: 423 proposed weaves, grouped by the milestone '

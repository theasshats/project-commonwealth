...
<!-- power-read: 0 blocks, 0 items, loot=no. Adds four Create-themed villager professions (Andesite Worker, Brass Worker, Copper Worker, Miner) with emerald-denominated trades for Create parts. Already 2-pillar. -->
<!-- existing connection REWORK check: Create anchor (sells Create parts; workstations are Create blocks) + economy anchor (trade economy). Both are sound functional edges. But — the economy anchor is currently emerald-denominated (vanilla economy), not Numismatics. The dossier flags this: "re-denominate from emeralds into Numismatics coins." This is a genuine REWORK candidate: the existing economy anchor works nominally but routes through the vanilla emerald system rather than the pack's actual economy (Numismatics coins). A player in this pack uses Numismatics, not emerald-as-currency. An emerald-denominated Create-parts trader is a partial island from the pack's intended economy loop. Flag as REWORK. -->
<!-- method-pull for depth: -->
<!-- candidate 1 (economy/M-08 coin re-denomination): replace emerald trades with Numismatics coin trades. The Andesite Worker could sell andesite-tier parts for Cogs/Spurs; the Brass Worker for higher-denomination coins. This uses KubeJS villager-trade modification, not a new recipe method — but it's a config/KubeJS delivery mechanism. Motif M-08 (coin from processed scarcity) — the raw materials the Miner accepts (raw copper/andesite) arrive via the scarcity chain, and paying coins instead of emeralds routes into the coin economy. ACCEPT — this is the primary deepening needed. -->
<!-- candidate 2 (survival/M-16): seasonal scarcity — certain Create parts are only available from the Brass Worker during a specific Serene Season (e.g. winter brass supply chain is disrupted, prices shift). This is a config-gate, not a recipe method. Motif M-16 applies if the trade stock (availability) is season-gated. Power: mid. Tone: ok — a seasonal trade disruption drives pressure → production → economy. But M-16 requires a season-gated *material input*, not a trade availability toggle — so this would need a KubeJS seasonal trade hook. Plausible but complex. ACCEPT as a candidate worth flagging even if the delivery is non-trivial. -->
<!-- candidate 3 (MineColonies integration): the Miner villager already trades raw materials. A MineColonies mine worker could also sell into this system — a "Create-specialized miner" profession for colonies. This isn't a method-pull — it's a system-design note. no-motif. REJECT. -->
REWORK: the `economy` anchor currently routes through vanilla emeralds rather than Numismatics. For a pack that uses coin-based economy, this is a partial disconnect — the Create-parts trade is real but the currency medium is wrong. Re-denominating trades into Numismatics coins would fully integrate this mod into the pack's economy loop.

- from: Create-parts trade (Andesite/Brass/Copper/Miner villagers) → re-denominated to Numismatics coins | via: KubeJS villager-trade modification | to: economy (coin loop) | motif: M-08 | power: mid | tone: ok | verdict: ACCEPT | hook: the brass worker doesn't deal in emeralds — he deals in cogwheels; re-price his trades in Cogs and Spurs so buying Create parts feeds the coin economy
- from: Brass Worker trade availability gated by Serene Season | via: KubeJS seasonal trade gate | to: survival | motif: M-16 | power: mid | tone: ok | verdict: ACCEPT | hook: winter disrupts the brass supply chain — the Brass Worker's stock is thinner in cold months, driving players to produce their own
- from: MineColonies miner profession feeding this villager's raw-material sink | via: config / system design | to: create | motif: no-motif | power: mid | tone: ok | verdict: REJECT | reason: no-motif — system-design note (two different worker types), not a method-routing recipe seam

...
Power-read: 43 blocks, 150 items, loot=yes, 11 biome-modifiers, 14 c:tags. Ores: jade, ruby, onyx, aquamarine, silver — all with deepslate variants and c:tags. Steel ingot (c:item/ingots/steel). Yokai drops: akaname_tongue, plus various mob drops from Oni/Tengu/Kitsune/Enenra. NOTE: samurai_dynasty silver must NOT be unified with galosphere (CLAUDE.md warning).

- from: samurai_dynasty:jade_ore / deepslate_jade_ore | via: create:crushing | to: Create | motif: M-03 | power: mid | tone: ok | verdict: ACCEPT | hook: Jade ore through the crusher yields jade dust and a chance at a second gem — the eastern ore seam feeds the same industrial pipeline as any western metal, making the samurai biomes a productive mining destination rather than just a combat zone.
- from: samurai_dynasty:ruby_ore / onyx_ore / aquamarine_ore | via: create:crushing | to: Create | motif: M-03 | power: mid | tone: ok | verdict: ACCEPT | hook: Ruby, onyx, and aquamarine ores each pass through the crusher for a doubled yield — scarce gemstones become more valuable to the Create spine when they're worth mining in quantity. Regional scarcity drives the specialization loop.
- from: samurai_dynasty:akaname_tongue (Yokai drop) | via: occultism:spirit_fire | to: magic | motif: M-11 | power: mid | tone: ok | verdict: ACCEPT | hook: The demon's tongue carries residual spirit energy; feeding it to occultism's spirit fire transmutes it into a dark reagent that Occultism rituals can consume. Yokai biology becomes arcane fuel — the folklore lineage makes this feel inevitable.
- from: samurai_dynasty:jade (gem) | via: ars_nouveau:imbuement | to: magic | motif: M-10 | power: mid | tone: ok | verdict: ACCEPT | hook: Jade holds spiritual significance in the mod's folklore aesthetic — immersing it in an Ars Nouveau enchanting apparatus infuses it with source and produces an arcane reagent with earth-aligned properties. The gem's lore anchors the infusion.
- from: samurai_dynasty:steel_ingot | via: create:pressing + create:mechanical_crafting | to: Create | motif: M-05 | power: mid | tone: ok | verdict: ACCEPT | hook: Steel sheets need precision work only the mechanical press can provide; samurai steel is a strong alloy that earns its place in Create's metal web. Its c:ingots/steel tag makes it a drop-in for any Create recipe that wants steel. Forge meets factory.
- from: samurai_dynasty Yokai mob drops (via biome-modifiers, loot=yes) | via: loot-seed + bountiful board | to: economy | motif: M-14 | power: mid | tone: ok | verdict: ACCEPT | hook: Bounty boards list Yokai hunts — an Oni head or Tengu feather is worth coin. The frontier danger (Yokai biomes = high-pressure zones) converts to economy income for the hunters who brave them.
- from: samurai_dynasty:silver_ore | via: create:crushing | to: Create | motif: M-03 | power: mid | tone: ok | verdict: REJECT | reason: CRITICAL — samurai_dynasty silver (samurai_dynasty:silver_ore / silver_block) must NOT be unified with occultism silver or galosphere palladium (CLAUDE.md silver caveat). Any create:crushing recipe that tags this into c:ingots/silver could create a cross-mod merge with the wrong material. The ore-doubling route is technically possible but requires explicit non-unification config; flagging as reject pending a safe tag-aware implementation. Do not author naively.
- from: samurai_dynasty katana / samurai_armor (finished gear) | via: numismatics vendor | to: economy | motif: M-09 | power: mid | tone: ok | verdict: REJECT | reason: Katanas and armor are personal combat equipment; pricing them as commodity goods in a vendor makes sense thematically but the samurai gear is self-crafted from own-mod ores — a vendor would just be selling the same bench recipe for coin, with no emergent scarcity (the ores are regional but the weapons aren't scarce once the player has the ores). The M-09 "high-effort processed good" framing doesn't apply to direct ore→weapon bench recipes. Reject; economy is better reached via M-14 Yokai bounties (above).

REWORK: survival (1) is correct; the mod has rich ore and mob-drop surfaces. New edges add Create (M-03 × 2, M-05), magic (M-11, M-10), economy (M-14) → survival + Create + magic + economy (4 anchors). Silver ore reject is an important safety flag.

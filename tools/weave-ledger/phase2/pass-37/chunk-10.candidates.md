...
- LEAVE — purely behavioral server-side AFK detector (0 blocks, 0 items, 0 recipe-types, loot=no). Idle-timer detection for sleep/mob-target exclusion; no materials or methods to route. Sanctioned server-admin QoL.

## create_tweaked_controllers   [anchors: aeronautics (1)]
- from: create_tweaked_controllers:tweaked_lectern_controller (mid–endgame control device, crafted from 2× linked controllers + electron tubes + precision mechanism) | via: create:sequenced_assembly or create:mechanical_crafting | to: create | motif: M-05 | power: mid | tone: ok — the tweaked controller is already built from Create's own precision mechanism and electron tubes; a Create-flavored recipe via mechanical_crafting or sequenced_assembly is entirely coherent with its industrial/control vibe | verdict: ACCEPT | hook: the tweaked controller's housing isn't hand-assembled — it's run through the assembly line like any precision Create instrument
- from: create_tweaked_controllers:tweaked_lectern_controller (mid) | via: thematic adjacency — it is a Create input device, one step from being a "drive a ship" controller | to: economy | motif: M-33 | power: mid | tone: ok — a skilled controller-operator offering "steering service" (cargo runs, passenger hops) is exactly M-33's labor axis; but the mod itself provides no mechanic that makes this an authored weave, only emergent play | verdict: REJECT | reason: M-33 is an emergent player-economy behavior, not authored via a recipe — the controller enables it but the weave can't be Phase-3 authored here; the link is ambient
- from: create_tweaked_controllers:tweaked_lectern_controller | via: M-24 drivetrain gating — controller as a propulsion-control input for aeronautics | to: aeronautics | motif: M-24 | power: mid | tone: ok — control surfaces/mechanisms are exactly M-24's domain; but the tweaked controller is already its own anchor as an aeronautics control device; using it as an ingredient *in* an aeronautics recipe would be the authored weave direction | verdict: ACCEPT | hook: an advanced Aeronautics navigation rig requires a tweaked controller as its input console — precision maneuvers, not a stick and a hope

## betterclouds   [anchors: support/client-visual (1)]
- LEAVE — client-only volumetric cloud renderer (0 blocks, 0 items, 0 recipe-types, loot=no). No in-world content surface. Sanctioned visual-polish support role.

## pantographsandwires   [anchors: create, aeronautics (2)]
REWORK: OK — connections sound. Create inbound weave is confirmed by 6 registered recipe-types (create:compacting, milling, mixing, mechanical_crafting, deploying, sequenced_assembly). Aeronautics anchor is appropriate — pantograph infrastructure is for electric trains. Depth (sequenced_assembly) is appropriate for a mid-tier railway component set.
- from: pantographsandwires:coal_coke / graphite (mid-tier intermediates; c:item/dusts/coal_coke, c:item/ingots/graphite) | via: tfmg:coking or create:crushing → as inputs to TFMG refining | to: create | motif: M-32 | power: mid | tone: ok — coal coke is a byproduct/intermediate in real-world coking; TFMG already has a `tfmg:coking` method, so routing pantographs' coal_coke dust through TFMG coking or as a TFMG coking feedstock is highly coherent and industrially authentic | verdict: ACCEPT | hook: the wire-making process generates coke as a byproduct — the blast furnace eats it, nothing wasted
- from: pantographsandwires:graphite (mid; c:item/ingots/graphite) | via: create_new_age:energising as a conductor intermediate or createaddition:rolling | to: create | motif: M-32 | power: mid | tone: ok — graphite is a real electrical conductor; Create New Age's energising method or Create Addition's rolling (wire production) are plausible routing methods for a graphite → conductor path that feeds the electric/FE chain | verdict: ACCEPT | hook: the graphite bar from the wire shop is the blank the energy grid's conductors are pressed from
- from: pantographsandwires:graphite | via: ars_nouveau:imbuement or occultism:crystallize as an arcane-conductor reagent | to: magic | motif: M-10 | power: mid | tone: clash — graphite is an industrial/tech material; pushing it into magic infusion as a "arcane conductor" is a stretch that requires a lore bridge neither Ars nor Occultism has for graphite specifically | verdict: REJECT | reason: tone clash; the industrial graphite context doesn't naturally bridge to arcane infusion without an invented lore hook; route graphite through industrial methods (M-32) where it belongs
- from: pantographsandwires catenary/mast blocks (mid deco-infra) | via: create:crushing → M-04 deco-recycle | to: create | motif: M-04 | power: everyday | tone: ok — metal catenary masts crush back to raw metal + XP nugget; standard lossy recycle | verdict: ACCEPT | hook: a torn-down overhead-line mast goes in the crusher and comes out scrap copper rod and an experience nugget

## formationsnether   [anchors: survival (1)]
- from: formationsnether structure loot tables (mid — nether-ruin contents) | via: loot-seed → drop magic reagents (ars_nouveau:source_gem dust, occultism ritual components, or Iron's arcane essence fragments) | to: magic | motif: M-02 | power: mid | tone: ok — nether altars and sanctuaries are thematically arcane/ritual spaces; seeding their chests with magic reagents makes the dangerous Nether feel like a source of occult power; a player "harvests" arcane drops from dangerous nether ruins | verdict: ACCEPT | hook: the ritual altar in the nether wastes wasn't abandoned — it still holds the soul-infused reagent the magic user needs
- from: formationsnether structure loot tables | via: loot-seed → drop boss-key components or rare Create metal (a fragment of a boss-key item or a regionally-scarce alloy shard) | to: create | motif: M-30 | power: mid–endgame | tone: ok — nether structures as a regional-scarcity node: scarce alloy fragments or partial boss-key drops locked in dangerous nether ruins drive players to explore rather than just mine; thematically the nether is the right place for rare-material caches | verdict: ACCEPT | hook: the only cache of ancient steel billets is inside a nether castle — you can mine for weeks or you can go find it
- from: formationsnether structure loot tables | via: loot-seed → player-mintable coin blanks or Numismatics raw coin material | to: economy | motif: M-08 | power: mid | tone: ok | verdict: REJECT | reason: M-08 requires the coin-minting step (Create processing → Numismatics mint) to be the valued weave — seeding pre-minted coins into chests would be an NPC-coin-faucet pattern akin to M-14 (cut); if anything, seed a Create-processable metal that can then be minted, which reduces to the M-30 accept above
- from: formationsnether structure loot tables | via: loot-seed → rare boss-drop fragment that functions as a combat-route supply | to: economy | motif: M-34 | power: endgame | tone: ok — combat specialists raid nether formations for high-value drops that non-combat players purchase; the nether danger makes these drops a natural traded commodity | verdict: ACCEPT | hook: the nether campsites drop what the piglin warlord left behind — rare enough that the combat specialist who cleared it can sell it for a week's supply

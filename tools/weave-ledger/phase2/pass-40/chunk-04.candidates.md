...
## tide
CHALLENGE | from: tide exotic fish (abyss_angler / void catches) | via: ars_nouveau:imbuement | to: magic | motif: M-10 | verdict: REJECT | hook: the M-10 fish→imbuement edge is rejected ~6× across the ledger and rightly so — Ars imbuement is for gems/crystals/essences, and a fish is a creature-drop, not an arcane material; the tone clash is real. The coherent magic home for genuinely-endgame void/lava catches is occultism:spirit_fire (M-11, transmuting a strange organic offering — already accepted for the truly-endgame fish). Uniformly drop M-10 for tide; keep M-11 scoped to void-fishing-tier catches only.
NEW | from: tide rare fish gated to a moon event (blood-moon / Ender-moon exclusive bites) | via: lunar-event spawn/catch gating → magic/Create method input | to: magic | motif: M-22 | power: mid | tone: ok | verdict: ACCEPT | hook: the ledger flags M-22 as wanting MORE coverage across every moon, and a fish that only bites under a specific moon is a textbook lunar reagent — it makes attending the moon event with a rod worthwhile and feeds that catch into a ritual/recipe (deepens the single thin M-22 row already present into a deliberate lunar-reagent gate spanning Enhanced Celestials + Ender Moon).

## mcwfences
OK — existing rows sufficient
(Decoration palette (180 blocks, sanctioned support anchor). The accumulated rows converge correctly: the only defensible Create edge is M-04 metal-fence crush-recycle (weak, lossy, do only as part of a wholesale Macaw's deco-family pass), wood-fence create:cutting is a minor mass-production convenience, the M-28 colony-fence-request edge is logged, and every hedge/organic edge is rightly rejected as forced. No missing routing — fencing is finished-form deco with no raw-material surface.)

## extradelight
NEW | from: extradelight feasts / high-tier dishes (cook processes another player's raw harvest) | via: cooking labor; payment emergent player-to-player | to: economy | motif: M-33 | power: mid | tone: ok | verdict: ACCEPT | hook: the deep-kitchen specialist cooks YOUR raw crops/meat into feasts and hands them back — value flows as labor (service-for-hire), distinct from the logged M-28 colony-provisioning (selling to the colony) and M-35 aging edges; it's the player-economy labor axis the kitchen is perfectly shaped for (a cook can't sell the capability, only the work).
NEW | from: extradelight:vat / melting_pot alcohol & reduced-fluid intermediates | via: alcohol_industry:alcohol_boiling / vinery ferment (aging) | to: economy | motif: M-35 | power: mid | tone: ok | verdict: ACCEPT | hook: the vat's alcohol bases and concentrated stocks must sit and ferment/age to gain value — routing them into the alcohol/vinery maturation chain creates a brewing-aging specialist niche (the value-adding time axis; broadens the drying_rack-only M-35 rows already present into the fluid/alcohol side of extradelight's stations).

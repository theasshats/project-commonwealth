...
Existing rows: M-05/M-06 (ultimate_trash_can gated on Create sequenced_assembly) has 1 ACCEPT each; all other proposals are REJECTs — the anti-scarcity nature of a void block makes most weaves incoherent.

The dossier says "none — support role; anti-economy/anti-scarcity." The M-06 gate idea (ultimate_trash_can gated on sequenced assembly) has exactly 1 ACCEPT from one pass. This deserves a clearer resolution:

The argument for M-06 gating: in a scarcity pack, *free infinite void* is a balance leak; gating it behind endgame-tier Create fabrication means players who want automation overflow relief must first invest in the Create spine — the gate is a balance measure, not a weave. The argument against: voiding is fundamentally anti-loop; even a well-gated trash can undermines M-32 (byproduct → input) design intent; the pack's philosophy should prefer making byproducts *useful*, not void-able.

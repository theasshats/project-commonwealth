...
Existing rows: all REJECT except M-33 painting-as-service (2/3 ACCEPT). The M-33 row argues painting as a "player performs work on another player's materials" service — but that description doesn't match: a painting is entirely self-contained (you paint your own image from any URL), it's not a specialist applying work to someone else's material (like an enchanter for hire). M-33 specifically requires "specialist performing *work* on another player's materials" — a painter doesn't do that.
...
Existing strong ACCEPTs: M-08 (titanium/tungsten → coin), M-13 (biofuel → aeronautics engine fuel), M-23 (martian_steel → airframe), M-24 (advanced_circuit + combustion_engine → drivetrain), M-06 (advanced_circuit via sequenced_assembly + engraving), M-37 (astronomical_reading → research/knowledge gate), M-22 (astronomical_reading → celestial ritual catalyst attempt, mostly REJECT), M-18 (Occultism miner → off-world dimension mining, 1 ACCEPT), M-29 (advanced_circuit as cross-route Create input), M-13/M-32 (biofuel → createaddition liquid_burning for FE). Coverage is extremely comprehensive.

What's missing:

1. **M-22 astronomical_reading** — mostly REJECTs and correctly so (navigation data ≠ arcane catalyst; M-22 requires a material *available only during a moon event*, not a star-observation tool). The 2 ACCEPTs for astronomical_reading → occultism/ars ritual are weak; they mistake "celestial theme" for the M-22 mechanism. No new angle here.

2. **M-16 seasonal / planet-atmosphere survival** — 1 ACCEPT row tries planet atmosphere → Cold Sweat survival requirement. This is the cleanest gap: each planet (Mars/Moon) has its own atmosphere requiring oxygen gear, cold protection, etc. These are survival pressure hooks. But M-16 specifically requires a *material obtainable only in a given Serene Season*; planetary atmosphere is not Serene Seasons. This doesn't fit M-16. The planet-survival constraint is better expressed as M-30 analog (dimension-locked scarcity) rather than M-16. The existing ACCEPT for "planet atmosphere → cold/breathable-air survival gear" is mis-motifed as M-30 (M-30 is GTMOGS regional ore-gen, not dimension-locked survival). No clean motif for "planetary survival constraint forces gear demand" unless via M-26 (consumption: oxygen gear wears out, continuous demand).

3. **M-26 oxygen/survival gear consumption** — not yet tried. If oxygen tanks or atmospheric concentrators require periodic refueling (biofuel/power), the consumption loop is real: off-world survival demands continuous oxygen supply, creating repeating demand for Northstar's biofuel. This connects the aeronautics/space pillar's survival-pressure back into the economy.

NEW | from: northstar:biofuel_bucket (renewable, produced via biomass processing) | via: createaddition:liquid_burning or northstar combustion_engine | to: survival | motif: M-26 | power: mid | tone: ok | verdict: ACCEPT | hook: off-world bases burn biofuel for atmospheric processing and warmth; the fuel isn't stockpiled — it's consumed continuously against the planetary environment's pressure, so the biofarm that feeds the space program also feeds the survival loop.

CHALLENGE | from: northstar:astronomical_reading | via: ars_nouveau:enchanting_apparatus or occultism:ritual | to: magic | motif: M-22 | verdict: REJECT confirmed — M-22 is "a material obtainable only during a moon event feeds a method"; astronomical_reading is an observational record produced at the astronomy_table at any time, not a material locked to a moon event. The 2 ACCEPTs in existing rows confuse thematic "celestial" adjacency with the mechanism. REJECT stands.

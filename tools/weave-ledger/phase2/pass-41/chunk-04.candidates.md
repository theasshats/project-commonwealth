...
- CHALLENGE | from: moblassos:hostile_lasso (spirit_fire/ritual rows that treat the filled hostile_lasso as a live ritual offering) | via: occultism:spirit_fire or occultism:ritual | to: magic | motif: M-11 | verdict: REJECT | hook: occultism:spirit_fire and occultism:ritual consume *items*, not container-NBT entities; a filled_lasso is an item with an entity embedded in its NBT — the method would consume the lasso item but the mob inside wouldn't be released or sacrificed in any way the recipe system can verify. The M-29 cross-route framing (hostile_lasso recipe requires both a Create-pressed clasp and an arcane imbuement cord) is the coherent authoring path; the live-mob-offering narrative is appealing but unimplementable via existing recipe types.

## create_tweaked_controllers   [anchors: aeronautics (1)]

Existing rows: M-05 (Create-part craft, strong accept), M-24 (drivetrain/control surface, strong accept). Coverage is the 1-anchor Create bridge — the existing rows confirm the craft routes through Create parts, earning the Create production anchor. What's missing: no M-37 angle, and no M-29 cross-route. The mod is genuinely thin on weave surface — its only item is the controller — but the M-37 gap is real: the "advanced control surface" that programs contraptions could plausibly require a MineColonies University research unlock (precision-engineering knowledge), making the recipe-capability a colony knowledge milestone rather than purely a material gate.

- NEW | from: create_tweaked_controllers:tweaked_lectern_controller (the only registered item — a precision-machined control surface requiring Create precision mechanism + electron tubes) | via: MineColonies University research gate (the "precision controls" or equivalent research unlocks the controller recipe — the knowledge isn't in the craft, it's in the colony's research tree) | to: economy | motif: M-37 | power: mid | tone: ok | verdict: ACCEPT | hook: anyone can mash a linked controller together, but the tweaked lectern controller — the one that maps physical inputs to a programmable frequency map — requires understanding precision tolerances that come from real engineering research, not just materials

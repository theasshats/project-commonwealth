...
Existing rows well-cover: M-12 (fan-sail processing of foreign crops/drops as survival link), M-16 (seasonal crop through freezing sail), M-24 (rubber → aeronautics seals/gaskets), M-23 (rubber as hull sealing component), M-19 (haunting_sail as M-19 bulk bridge). The M-31 (cardboard packages as logistics bulk) has a 2/2 accept split. What no pass has examined: the dndesires:industrial_fan as a *propulsion-relevant component* for aeronautics (its high-RPM fan head as a propeller nacelle component, M-24), which 1 pass named but the convergence is 1 ACCEPT total and deserves sharper framing.

- NEW | from: dndesires:industrial_fan (large high-RPM fan block — a Create-class mechanical component with a large fan diameter) | via: crafting recipe requiring industrial_fan as an ingredient in an Aeronautics propeller or thruster assembly | to: aeronautics | motif: M-24 | power: mid | tone: ok | verdict: ACCEPT | hook: the industrial fan head is the pack's largest Create-native fan — scaling up from a standard encased fan, the ship engineer bolts an industrial fan nacelle into the propulsion assembly for a heavier airship; the physical fan-as-propeller link is thematically exact and doesn't require inventing a new recipe type (aeronautics construction recipe already accepts component inputs)

...
- NEW | from: Terralith Skylands biome (floating islands — structurally inaccessible without flight or dedicated bridging effort) | via: aeronautics — the Skylands are only economically farmable by a player with a functional airship; seeds/blocks/seeded loot there are de-facto aeronautics-gated | to: aeronautics | motif: M-31 | power: mid | tone: ok | verdict: ACCEPT | hook: the Skylands show up on the map but nobody harvests them consistently until the aeronautics arm is operational — terrain as the logistics demand driver, not recipe weight
- NEW | from: Terralith biome-exclusive fantastical terrain (Moonlight Grove, volcanic biomes, cave biomes) seeded with a biome-specific seasonal plant via loot-seed or worldgen config | via: loot-seed + Serene Seasons config — a unique flower/herb in the Moonlight Grove grows only in autumn, combining M-30 (biome-locked) and M-16 (seasonal) into a double-gated reagent | to: magic | motif: M-16 | power: mid | tone: ok | verdict: ACCEPT | hook: the night-bloom herb grows only in the Moonlight Grove and only in autumn — the alchemist needs both the terrain access and the calendar window, making it the pack's hardest-to-source seasonal reagent

## gravestone   [anchors: survival (1)]
- LEAVE — death-QoL utility with no craftable material surface; all recipe/magic weave attempts correctly rejected; the one surviving M-33 angle (danger-zone grave recovery as a paid service) is situationally valid on a small server but requires no authoring — it emerges from the obituary's coordinate data + player danger-avoidance naturally, with no Phase 3 recipe work to do; leave the emergence organic

...
- from: expandeddelight:salt_ore / deepslate_salt_ore (item/ores/salt) | via: create:crushing | to: create | motif: M-03 | power: everyday | tone: ok | verdict: ACCEPT | hook: salt ore is a new worldgen ore — feeding it through Create's Crushing Wheels gives salt dust + a small secondary drop (e.g. a mineral flake byproduct), exactly the ore-doubling pattern M-03 is made for; it ties expandeddelight's key ingredient to the Create spine (a salt specialist runs a crushing setup, not just a vanilla furnace). One light step, everyday tier. The c:ores/salt tag makes this tag-compatible with existing Create ore-crushing config patterns.
- from: expandeddelight:cinnamon bark / item/dusts/cinnamon | via: create:milling | to: create | motif: M-12 | power: everyday | tone: ok | verdict: ACCEPT | hook: cinnamon bark → cinnamon dust via Create Millstone is a natural processing-chain pull: a baker or spice trader needs ground cinnamon, which the Create milling step provides, tying the farming chain (grow cinnamon tree → harvest bark → mill dust → bake) into the Create spine as a short multi-step chain. Everyday — one Millstone step.
- from: expandeddelight:cheese_wheel (time-gated, cask-matured) | via: M-35 maturation/aging | to: economy | motif: M-35 | power: mid | tone: ok | verdict: ACCEPT | hook: cheese wheels must age in casks before use — a maturing specialist holds inventory in process, can't sell until ready; aged cheese is inherently time-scarce (you can't rush it), so it commands a price premium in the player economy and creates a genuine aging-specialist role. M-35 (maturation creates value-adding time axis) is exactly the motif. Via: the existing expandeddelight:cask mechanic; no recipe to author — the delivery is design-level (document cheese as the pack's canonical maturation good, price it in vendors accordingly in Phase 3).
- from: expandeddelight:salt_ore | via: M-30 regional-scarcity gate | to: economy | motif: M-30 | power: everyday | tone: ok | verdict: REJECT | reason: salt is in the game via ore gen, which is good, but calling it a "regional-scarcity gate" requires GTMOGS to specifically assign salt to certain regions only — that is a GTMOGS configuration decision, not something expandeddelight controls. If the pack's ore-gen doesn't region-lock salt, there is no regional scarcity gate. This is a GTMOGS config task, not an expandeddelight weave. REJECT as a weave on this mod; note it as a GTMOGS configuration candidate instead.
- from: expandeddelight juicer output (apple_juice, cranberry juice, etc.) | via: vinery:apple_mashing / alcohol_industry:alcohol_boiling | to: economy | motif: M-35 | power: mid | tone: ok | verdict: REJECT | reason: juices feeding a fermentation/aging chain is a real M-35 scenario, but expandeddelight's juicer already overlaps with vinery/alcohol_industry's own fruit inputs — adding a juicer→ferment chain duplicates the vinery/alcohol chains. The cleaner weave is to let vinery own fermentation and expandeddelight own the juicer for non-alcoholic drinks. Duplication risk; REJECT to avoid fragmentation.
REWORK: dossier's economy candidate for cheese uses M-09 (retired) framing ("aged cheese wheels & salt as a tradeable/regional luxury… a natural sellable"). The M-35 accept above replaces it correctly — the weave is the aging mechanic creating value, not a bare sell link. No other connections to REWORK.

## polymorph   [anchors: support/compat (1)]
- LEAVE — recipe-conflict UI resolver; 0 blocks, 0 items, no recipe type of its own; pure QoL compat infrastructure. Nothing to weave.

## createfurnitureseats   [anchors: support/compat + aeronautics (1–2)]
- LEAVE — pure behavior-bridge mod; 0 blocks, 0 items, no recipe surface. It tags furniture from other mods as rideable seats on Create/Sable contraptions — this is already the aeronautics-compat weave its whole existence delivers. No material to route; forcing a recipe edge onto a compat glue mod with no content is a defect. Already earns its aeronautics anchor by definition.

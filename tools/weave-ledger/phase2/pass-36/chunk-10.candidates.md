...
- from: friendsandfoes:crab_claw | via: occultism:spirit_fire | to: magic | motif: M-11 | power: mid | tone: ok | verdict: ACCEPT | hook: a crustacean claw dissolved through spirit-fire produces a tidal or oceanic essence — aquatic combat loot transmuted into occult reagent; the aquatic/spirit link is thematically coherent (the sea and spirit worlds share mystery)
- from: friendsandfoes:crab_claw (→ vanilla reach potion via brewing) | via: irons_spellbooks:alchemist_cauldron_brew | to: magic | motif: M-10 | power: mid | tone: ok | verdict: ACCEPT | hook: the crab claw already brews a reach potion via vanilla; routing it through Iron's Spellbooks alchemist cauldron as an alternative (producing a reach-scroll or enhanced brew) extends the combat-loot→magic path without displacing the vanilla route
- from: friendsandfoes copper set (copper buttons / lightning rods / copper golem head) | via: create:crushing | to: create | motif: M-04 | power: everyday | tone: ok | verdict: ACCEPT | hook: decorative copper pieces from the copper golem set crush back to copper nuggets and gravel — the Create deco-recycles-to-raw motif applied cleanly to a copper-themed deco block set; sensible, low-cost, everyday weave
- from: friendsandfoes:wildfire drops (hostile mob — if any) | via: ars_nouveau:imbuement | to: magic | motif: M-02 | power: mid | tone: ok | verdict: ACCEPT | hook: the Wildfire is a fire-elemental hostile; its drops fed through arcane imbuement as a flame-reagent is a coherent mob→magic sink (fire elemental → fire catalyst, something any player would nod at)
- from: friendsandfoes:moobloom (floral cow) | via: create:milling | to: create | motif: M-12 | power: everyday | tone: ok | verdict: REJECT | reason: Moobloom drops flowers when sheared — milling flowers is already covered by vanilla/Create; routing Moobloom flowers through a mill is too thin and provides no distinct cross-system value, it's just "flowers → dye/powder" which exists without this mod
- from: friendsandfoes:copper_golem_head | via: occultism:ritual | to: magic | motif: M-11 | power: mid | tone: ok | verdict: REJECT | reason: the copper golem head is a decorative trophy/skull, and Occultism rituals consuming golem heads has no thematic anchor in occultism's lore (spirit-fire/ritual focus on organic/soul materials, not mechanical copper); tone clash
- REWORK: dossier's "leave acceptable" note on the copper-deco M-04 candidate undersells it — M-04 on the copper set is a clean, coherent everyday weave that should ACCEPT; the magic M-02 paths via crab_claw and Wildfire are also coherent and should not be dismissed as thin

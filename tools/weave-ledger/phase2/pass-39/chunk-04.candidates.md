...
Power-read: 16 blocks, 81 items, 1 biome-modifier, loot=yes. Full content surface: Soul Furnace (registered recipe-type `companions:soul_furnace`), summon altars, Tesla electrical network, boss (Sacred Pontiff drops Old Cloth + Relic Gold), tamable companions, hostile mobs (drop Small/Great Essence + Crystallized Blood), its own copper/nether/end coins. Large standalone mod.

Method-pull candidates:
- from: companions:crystallized_blood, companions soul gems, companions essences | via: occultism:ritual or ars_nouveau:imbuement | to: magic | motif: M-11 | power: mid | tone: ok | verdict: ACCEPT | hook: Crystallized Blood (produced in the Soul Furnace from mob drops) and soul gems are direct analogs to the kind of spirit-state reagent Occultism traffics in — routing them into Occultism rituals as a required ingredient (or as feedstock for the spirit-fire transmutation) connects the Soul Furnace economy to the magic web; a Companions player becomes a supplier to an Occultism mage. The Soul Furnace's mob-drop→soul-state transformation is thematically adjacent to Occultism's spirit mechanics; tone fits strongly. One ritual step using Crystallized Blood as a reagent. via: occultism:ritual.
- from: companions boss drop: Relic Gold | via: create:mechanical_crafting (gated recipe) | to: create | motif: M-15 | power: endgame | tone: ok | verdict: ACCEPT | hook: Relic Gold (dropped only by the Sacred Pontiff boss) is a boss-key gate item for a high-tier Create recipe (e.g. a specialized machine component or Aeronautics controller part) — the fighter who kills the Sacred Pontiff becomes the supplier of a crafting ingredient the tech specialist needs. The Pontiff's gothic-alchemical theme is consistent with guarding a rare metal; "Relic" implies age and rarity. Endgame tier; one or two recipe steps using Relic Gold as an input component. via: create:mechanical_crafting.
- from: companions:copper_coin / nether_coin / end_coin | via: numismatics | to: economy | motif: M-08 | power: everyday | tone: clash | verdict: REJECT | reason: Companions ships its own copper/nether/end coins as an in-mod economy separate from Numismatics. Bridging these to Numismatics would require either a conversion recipe (making the companion coins a coin-blank source) or a direct substitute, but two parallel coin systems competing for the "money" role fragment the economy rather than consolidating it. The safer path is to keep Companions' coins internal (tips, companion upgrades) and not bridge them into the Numismatics layer. Tone: the gothic-alchemical coin aesthetic clashes with Numismatics' industrial-currency aesthetic. REJECT (parallel coin systems fragment the economy; tone mismatch; potential for confusion about what "money" is).
- from: companions Tesla network (voltaic pillars/coils) | via: create:crushing | to: create | motif: M-04 | power: everyday | tone: ok | verdict: ACCEPT | hook: Tesla/voltaic deco blocks (voltaic_pillar, tesla_coil_block, voltaic_relay_block) crush back to copper nuggets + copper scrap, folding the electrical deco set into Create's copper recycling loop — consistent with M-04 (deco recycles back to raw material, lossily). Everyday tier; one crushing step. This is a light but coherent link that prevents the electrical deco from being a dead-end. via: create:crushing.
- from: companions mob drops (antlion_fur, blade_arm, cannon_arm etc.) | via: create:mechanical_crafting | to: create | motif: M-02 | power: mid | tone: ok | verdict: REJECT | reason: M-02 is the mob-drop-reagent-sink motif routed through a magic/economy method, not a Create mechanical_crafting method. Routing Companions mob drops into Create crafting as mechanical components doesn't fit M-02 (which is organic→magic/economy), and there's no other motif that covers "mob drop as Create machine input" — that would be closer to M-15 (boss key) but for regular mob drops, which dilutes the boss-key rarity principle. REJECT (no-motif for the organic→Create route with regular mob drops; M-15 already covers the boss-drop layer, accepted above).

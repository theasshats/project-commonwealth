...
> `docs/MODLIST-AUDIT.md` fit-flags (§A) / scarcity (§C) / dup (§B), and the in-flight curation issues
> (#83 / #94 / #100 / #113 / #45 / #60). Lands in **v0.6.0** (Foundation) as the charter; every later
> pass runs against it. Pairs with `docs/RELEASE-CADENCE.md` (odd/even), #160 (conflict catalog), #161
> (mod-update pass).
...
1. **System anchor (the north star).** It feeds one of the five systems — **Create, magic, economy,
   aeronautics/logistics, survival & seasons** — *or* a sanctioned support role: **library/API,
   performance, QoL, decoration palette, or curated friend-group flavor**. Anchors to nothing → **cut**.
   (`docs/DESIGN.md`; the "if none, why is it here?" test. The DESIGN goal is *two* pillars — one is the
   floor, so a one-pillar mod is a candidate for a second weave, not a cut.)
...
_Refs: `docs/DESIGN.md` (the north star), `docs/MODLIST-AUDIT.md` (fit-flags §A / scarcity §C / dup §B),
`docs/CONNECTIVITY.md`, `docs/ROADMAP.md` (the pass schedule), `docs/RELEASE-CADENCE.md` (odd/even),

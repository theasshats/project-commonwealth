...
## The goal — five systems in one loop, every mod earns its place
...
> **The five systems are not flat peers — they form one causal loop.** The structural model (inspired by the game *Eco*) is **scarcity → pressure → production → distribution**, and back: forced specialization producing an emergent, load-bearing player economy. **`docs/SYSTEMS.md` is the canonical structural model** — read it for how the parts interlock (producers, connectors, pressure, the ecological cost, the recipe/gating spine). This section is the *why*; SYSTEMS.md is the *how it fits*.
...
1. **Create spine** — the tech core; things made through Create. *A producer.*
2. **Magic web** — Ars Nouveau / Iron's Spellbooks / Occultism, bridged into one progression. *A producer, with its own locked exclusives.*
3. **Economy** — Numismatics + Trading Floor + bounties: the trade layer. *A connector — it moves value. Not optional flavor: it's the load-bearing consequence of production exclusivity (see below).*
4. **Aeronautics & logistics** — Create Aeronautics ships, trains, drones; the flight/transport ladder. The pack's flagship content. *A connector — it moves matter — built through Create (a producer⇄connector hybrid), and the system that most threatens regional scarcity, so it carries its own balance lens.*
5. **Survival** — Cold Sweat × diet variety (Diet) × the hostile world: the world pushing back. *The pressure that generates demand.* (Serene **seasons** is a driver feeding both scarcity and survival, not a standalone system.)

Under all of it sits **scarcity** (regional ore-gen) — the foundation; and two cross-cutting **production routes**, **MineColonies** (cheaper basics + locked exclusives, across tech *and* magic) and **boss drops** (high-tier components), make the producers non-overlapping so trade is necessary. The full structure is in `docs/SYSTEMS.md`.
...
**Specialization & trade — emergent in mechanism, load-bearing by design.** With ~10 players and scarce regional ores, no one can comfortably solo every system (~3 people to cover all production). Trade emerges from that — people running MineColonies at different rates, different regions holding different ores, each production route holding **locked exclusives** only it can make — rather than from an enforced trade mechanic. So the economy is **emergent** (nothing forces you to trade) but **no longer "optional flavor"**: the design leans on it, because hard base recipes (~3× basics) plus locked exclusives make going solo impractical. Specialization is **soft for now** (a determined player or bloc *can* eventually cover everything; revisit near release if play shows the loop eroding — #224), and the hardening lever, if needed, is the lock list.

**Progression on-ramps.** Complex Create tech unlocks through **MineColonies** (settlement progress) or **boss drops** — weaving colonies and combat *into* the tech tree as on-ramps, rather than leaving them as parallel side-games. The high-tier fork (a top item needs a boss drop *or* a colony-crafted part) and the locking are implemented in **KubeJS**, with the lock list defined incrementally per pillar (#220). MineColonies is a first-class production route, not "a rival economy to disable."

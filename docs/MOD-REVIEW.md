...
**These reviews *are* the thunderdomes.** The odd-milestone curation-by-merit (`docs/ROADMAP.md`) runs *through* this sweep: each per-mod review is a **keep / cut / weave** judgement, and the evidence it judges against — every mod's candidate connections from the ~5,850-candidate weave-ledger pass — is integrated in **[`docs/WEAVE-REVIEW.md`](WEAVE-REVIEW.md)** (regenerated by `tools/weave-ledger/gen-weave-review.py`, with a DX take on each mod). Read that doc alongside this checklist when a milestone's batch comes up.

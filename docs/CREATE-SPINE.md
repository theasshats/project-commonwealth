...
| **T4** | boss metals (`ignitium`, `cursium`) · `#c:ingots/silver` | The boss gates (Part 3) — plus **silver, the cross-region ingredient**: `c:ingots/silver` resolves to `occultism:silver_ingot`, smelted from the occult mod's ore — which generates only in silver country (hills/rocky mountains) plus trace bands. The T4 generator build (layered magnet) therefore pulls a region most tech bases don't hold: mine it or trade for it. (A true producer-locked magic↔tech seam — a part only magic *crafting* makes — is the v0.11.0 follow-up.) Never let galosphere "silver" satisfy it — that item is palladium. |
